...
static PyObject* process(PyObject* self, PyObject *args)
{
    ...
    if(PyArray_DESCR(input_array)->type_num != NPY_FLOAT || PyArray_DESCR(output_array)->type_num != NPY_FLOAT) {
        PyErr_SetString(PyExc_ValueError, "arrays must be of type float");
        return NULL;
    }
    if(PyArray_NDIM(input_array) != 2 || PyArray_NDIM(output_array) != 2) {
        printf("%d, %d\n", PyArray_NDIM(input_array),
                PyArray_NDIM(output_array));
        PyErr_SetString(PyExc_ValueError, "arrays must be two dimensional");
        return NULL;
    }
    if((client->num_inputs > 0 && PyArray_DIMS(input_array)[1] != client->buffer_size) || 
       (client->num_outputs > 0 && PyArray_DIMS(output_array)[1] != client->buffer_size)) {
        PyErr_SetString(PyExc_ValueError, "columns of arrays must match buffer size.");
        return NULL;
    }
    if(client->num_inputs > 0 && PyArray_DIMS(input_array)[0] != client->num_inputs) {
        PyErr_SetString(PyExc_ValueError, "rows for input array must match number of input ports");
        return NULL;
    }
    if(client->num_outputs > 0 && PyArray_DIMS(output_array)[0] != client->num_outputs) {
    ...
    if (client->input_buffer_size) {
        ...
        for(c = 0; c < client->num_inputs; c++) {
            for(j = 0; j < client->buffer_size; j++) {
                memcpy(
                    PyArray_DATA(input_array) + (c*PyArray_STRIDES(input_array)[0] + j*PyArray_STRIDES(input_array)[1]), 
        ...
        for(c = 0; c < client->num_outputs; c++) {
            for(j = 0; j < client->buffer_size; j++) {
                memcpy(&client->output_buffer_1[j + (c*client->buffer_size)],
                       PyArray_DATA(output_array) + c*PyArray_STRIDES(output_array)[0] + j*PyArray_STRIDES(output_array)[1],
                       sizeof(float)
                );
            }
        }
